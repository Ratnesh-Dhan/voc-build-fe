...
const Moviesources = ({word, number}) => {
  const [metadata, setMetadata] = useState<{data: IMetadata; error: boolean}>({data: null, error: false});
  const [Number] = useState(number);

  const movieInfo = async(word, number) => {
    await axios.get(`https://api.vocbuild.com/movie/clip/metadata/${word}/${number}`).then((response)=>{
    ...
    <div className="bg-blue-200 p-3">
        ...
        <a href={`https://imdb.com/title/${metadata?.data?.id}`} target="_blank">
          <p className="text-sm mt-2">
            IMDB : <span className="text-blue-900 hover:underline">{`https://imdb.com/title/${metadata?.data?.id}`}</span> 
          </p>
        </a>
